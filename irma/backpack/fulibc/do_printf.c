...
int do_printf(const char *fmt, va_list args, fnptr_t fn, void *ptr)
{
	...
	for (; *fmt; fmt++) {
		...
		case 0:
			...
			/* found %, get next char and advance state to check
			 * if next char is a flag */
			state++;
			fmt++;
			/* FALL THROUGH */
		...
		case 1:
			...
			/* not a flag char: advance state to check if it's
			 * field width */
			state++;
		...
		case 2:
			...
			/* not field width: advance state to check if it's
			 * a modifier */
			state++;
			/* FALL THROUGH */
		...
		case 3:
			...
			/* not modifier: advance state to check if it's
			 * a conversion char */
			state++;
			/* FALL THROUGH */
		...
		case 4:
			...
			*where = '\0';
			...
			case 'X':
				...
				/* xxx - far pointers (%Fp, %Fn) not yet
				 * supported */
			case 'x':
			case 'p':
			case 'n':
...
DO_NUM:
				...
				/* no h nor l: sizeof(int) bits (signed
				 * or unsigned) */
				else {
					if (flags & PR_SG)
						num = va_arg(args, int);
				 ...
				 * OK, I found my mistake. The math here is
				 * _always_ unsigned */
				do {
					unsigned long temp;
