...
void Flash_Handler(SIGNAL *s)
{
    if (s->sig_no == SIG_FLASH_WRITE) {
        ...
        *((unsigned int *)0x00800314) |= 0x10;
        ...
        SIGNAL *r = alloc(4, SIG_FLASH_COMPLETE);
        send(&r, sender(&s));
    } else {
        UserError(0x20);
    }
...
void Flash_ErasePage(void *addr)
{
    ...
    SIGNAL *s = alloc(sizeof(struct sig_flash_erase), SIG_FLASH_ERASE);
    struct sig_flash_erase *p = (void *) s;
    p->addr = addr;
    send(&s, proc_flash_eraser);

    //printf("%s: signal sent\n", __FUNCTION__);

    const SIGSELECT response[] = {1, SIG_FLASH_COMPLETE};
    s = receive((SIGSELECT *) response);
    free_buf(&s);
...
{
    SIGNAL *s = alloc(sizeof(struct sig_flash_write), SIG_FLASH_WRITE);
    struct sig_flash_write *p = (void *) s;
    p->dst = dst;
    p->src = src;
    p->len = len;
    send(&s, proc_flash_handler);
